# iPhone Manager bot originally by Oldmole, edited by karenleewaddell.
...
# The use of the software and scripts is done at your own discretion and risk.
...
async def shutdown_command(params, message):
        params = ''.join(params)
        for x in iphone_list:
                if params == x.iphone_name or params == x.iphone_id:
                        a = x.iphone_name
                        b = x.device_uuid
                        cp = subprocess.run(["idevicediagnostics", "-u", b, "shutdown"], universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        if cp.returncode == 0:
                                await message.channel.send("%s is shutting down" % a)
...
async def on_ready():
	...
	activity = discord.Game(name="DevicesBot")	
